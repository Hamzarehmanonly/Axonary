...
import { EcommercePlatformSVG, PaymentGatewaySVG, InventoryManagementSVG, EcommerceMarketingSVG, MobileCommerceSVG } from "../../../components/IndustriesSVGs.js";
...
const services = [
  { id: "platforms", title: "E-Commerce Platforms", desc: "Build scalable online stores with product management, shopping carts, and checkout systems." },
  { id: "payments", title: "Payment Gateway Integration", desc: "Secure payment processing with multiple gateways, wallets, and buy-now-pay-later options." },
  { id: "inventory", title: "Inventory Management", desc: "Real-time inventory tracking, warehouse management, and automated restocking." },
  { id: "marketing", title: "E-Commerce Marketing Automation", desc: "Email campaigns, personalized recommendations, and customer segmentation for increased sales." },
  { id: "mobile-commerce", title: "Mobile Commerce Solutions", desc: "Native mobile shopping apps with push notifications, mobile payments, and AR try-on features." },
];
  ...
  <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-sm" style={{ background: "rgba(0,0,0,0.45)", borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
    <div className="container mx-auto px-6 flex items-center justify-between h-16">
      <div className="flex items-center gap-3">
        ...
        <div><div className="text-sm font-bold" style={{ color: COLORS.white }}>Axonary</div><div className="text-xs" style={{ color: COLORS.textMuted }}>E-Commerce Solutions</div></div>
      </div>
      <nav className="hidden md:flex items-center gap-6 text-sm" style={{ color: COLORS.textMuted }}>
        <a href="#platforms" className="hover:text-white">Platforms</a><a href="#payments" className="hover:text-white">Payments</a><a href="#inventory" className="hover:text-white">Inventory</a><a href="#marketing" className="hover:text-white">Marketing</a><a href="#mobile-commerce" className="hover:text-white">Mobile</a>
    ...
    <div className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
      <div className="md:col-span-7 flex justify-center">
        <div className="text-left">
          <div className="inline-block rounded-full px-4 py-1 text-sm font-medium" style={{ background: `${COLORS.secondary}33`, color: COLORS.white }}>Scalable • Secure • Converting</div>
          <h1 className="mt-6 text-4xl md:text-6xl font-extrabold leading-tight">E-Commerce <span style={{ color: COLORS.primary }}>Redefined</span></h1>
          <p className="mt-4 text-lg text-gray-300 max-w-2xl">Build powerful online stores with seamless checkout, inventory management, and customer engagement.</p>
          <div className="mt-6 flex items-center gap-4">
            <a href="#platforms" className="px-6 py-3 rounded-full font-semibold" style={{ background: COLORS.primary, color: COLORS.white }}>Explore Solutions</a>
      ...
      <div className="md:col-span-5"><div className="rounded-2xl p-6" style={{ background: COLORS.card }}><EcommercePlatformSVG width={360} height={360} /></div></div>
    </div>
  </section>
);

const SplitSection: React.FC<{ idx: number; id: string; title: string; desc: string; }> = ({ idx, id, title, desc }) => {
  const getSVG = () => [<EcommercePlatformSVG width={360} height={360} />, <PaymentGatewaySVG width={360} height={360} />, <InventoryManagementSVG width={360} height={360} />, <EcommerceMarketingSVG width={360} height={360} />, <MobileCommerceSVG width={360} height={360} />][idx];
